...
type Backend struct {
	...
	// Set when the panel is adding another core to a node it is already
	// connected to, rather than connecting to it. A plain Start resets the node
	// and replaces whatever it was running; an additive one leaves the cores
	// that are already up alone.
	Additive      bool `protobuf:"varint,6,opt,name=additive,proto3" json:"additive,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *Backend) GetAdditive() bool {
	if x != nil {
		return x.Additive
	}
	return false
...
const file_common_service_proto_rawDesc = "" +
	...
	"\fnode_version\x18\x03 \x01(\tR\vnodeVersion\"\xd6\x01\n" +
	"\aBackend\x12(\n" +
	...
	"\x10exclude_inbounds\x18\x05 \x03(\tR\x0fexcludeInbounds\x12\x1a\n" +
	"\badditive\x18\x06 \x01(\bR\badditive\"\x1d\n" +
	"\x03Log\x12\x16\n" +
