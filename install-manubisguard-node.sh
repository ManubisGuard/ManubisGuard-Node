...
NODE_INSTALLER_RAW="https://raw.githubusercontent.com/ManubisGuard/ManubisGuard-Node/feature/amnezia-wg/install-manubisguard-node.sh"
...
install_node_script() {
    ...
    if ! github_download_file "$NODE_INSTALLER_RAW" "$TEMP_FILE"; then
        colorized_echo red "✗ Failed to download script from $NODE_INSTALLER_RAW"
        rm -f "$TEMP_FILE"
        exit 1
    fi
...
install_node() {
    ...
    colorized_echo cyan "  Command: curl -fsL $COMPOSE_FILES_URL_PREFIX/docker-compose.yml -o $APP_DIR/docker-compose.yml"
    if curl -fsL "$COMPOSE_FILES_URL_PREFIX/docker-compose.yml" -o "$APP_DIR/docker-compose.yml"; then
...
update_node_script() {
    ...
    local temp_script
    temp_script=$(create_temp_file "manubis-node-script" ".sh")
    if ! github_download_file "$NODE_INSTALLER_RAW" "$temp_script"; then
    ...
    sed -i "s|^APP_NAME=.*|APP_NAME="$APP_NAME"|" "$temp_script"
    install -m 755 "$temp_script" "/usr/local/bin/$APP_NAME"
    rm -f "$temp_script"
